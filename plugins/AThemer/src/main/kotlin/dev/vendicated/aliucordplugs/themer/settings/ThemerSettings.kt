...
class ThemerSettings : SettingsPage() {
    ...
    override fun onViewBound(view: View) {
        ...
        if (AThemer.mSettings.fontHookCausedCrash) {
            AThemer.mSettings.fontHookCausedCrash = false
            ConfirmDialog()
                .setTitle("Oops!")
...
/*        TextView(ctx, null, 0, R.i.UiKit_TextView).run {
            ...
            SpannableStringBuilder(content).let {
                it.setSpan(object : ClickableSpan() {
                    override fun onClick(widget: View) {
                        val manifest = PluginManager.plugins["AThemer"]!!.manifest
        ...
        arrayListOf(
            ...
            manager.a(radios[AThemer.mSettings.transparencyMode.value])
            for (i in 0 until radios.size) {
                val radio = radios[i]
                radio.e {
                    manager.a(radio)
                    AThemer.mSettings.transparencyMode = TransparencyMode.from(i)
        ...
        addView(
            Utils.createCheckedSetting(
                ...
                isChecked = AThemer.mSettings.enableFontHook
                setOnCheckedListener { checked ->
                    if (!checked) {
                        AThemer.mSettings.enableFontHook = false
                        return@setOnCheckedListener
                    }
                    isChecked = false
                    ConfirmDialog().apply {
                        setTitle("Hold on")
                        setDescription("This is unstable on some roms and may lead to crashes or the Aliucord settings section to disappear.\nIf such a crash occurs, fonts will automatically be disabled again.\n\nIf this for some reason fails or only the settings sections disappears, you must manually open the settings folder in your Aliucord directory and delete 'AThemer.json' to fix it.\nPROCEED AT YOUR OWN RISK!")
                        setIsDangerous(true)
                        setOnOkListener {
                            isChecked = true
                            AThemer.mSettings.enableFontHook = true
        ...
        addView(
            Utils.createCheckedSetting(ctx, CheckedSetting.ViewType.SWITCH, "Enable Custom Sounds", "Enable support for custom sounds")
                .apply {
                    isChecked = AThemer.mSettings.customSounds
                    setOnCheckedListener {
                        AThemer.mSettings.customSounds = it
                        promptRestart(view, this@ThemerSettings)
                    }
                }
        )
