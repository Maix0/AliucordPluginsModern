...
fun addPatches(patcher: PatcherAPI) {
    patcher.run {
        if (AThemer.mSettings.transparencyMode != TransparencyMode.NONE) setBackgrounds()

        if (AThemer.mSettings.enableFontHook) patchGetFont()

        if (AThemer.mSettings.customSounds) patchOpenRawResource()
 ...
 * Here we deoptimise (un-inline) a few that prevent AThemer from doing its job.
 */
fun deoptimize() {
