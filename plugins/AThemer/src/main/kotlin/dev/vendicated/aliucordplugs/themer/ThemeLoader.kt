...
object ThemeLoader {
    ...
    private fun getThemeCacheDir(theme: Theme): File {
        val cacheDir = File(Utils.appContext.cacheDir, "AThemer-Cache").also {
    ...
    private fun getResourceWithCache(theme: Theme, name: String, url: String): File {
        ...
        if (!file.exists() || AThemer.mSettings.getString(cachePrefKey, "") != url) {
            // Enforce privacy
            ...
            try {
                Utils.threadPool.submit {
                    Http.Request(url).use {
                        ...
                        AThemer.mSettings.setString(cachePrefKey, url)
                    }
                }
